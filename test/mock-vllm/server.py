...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if length > 0:
                ...
                self._json(400, {"error": "missing Content-Length and Transfer-Encoding"})
                return
            if not raw:
    ...
    def log_message(self, fmt, *args):
        print(f"[mock-vllm] {fmt % args}", flush=True)
...
if __name__ == "__main__":
    ...
    if os.path.exists(CERT_FILE) and os.path.exists(KEY_FILE):
        try:
            ...
            print(f"[mock-vllm] Starting HTTPS on port {PORT}", flush=True)
        except (ssl.SSLError, PermissionError, OSError) as e:
            print(f"[mock-vllm] TLS setup failed ({e}), falling back to HTTP on port {PORT}", flush=True)
    else:
        print(f"[mock-vllm] No certs found, starting HTTP on port {PORT}", flush=True)
